...
fn render_command_list(frame: &mut Frame, area: Rect, app: &App) {
    let favorites = app
        .config
        .as_ref()
        .map(|c| c.artisan_favorites())
        .unwrap_or(&[]);
    let commands = app.artisan_tab.current_command_display(favorites);

    let title = format!(" Artisan Commands ({}) ", commands.len());
    ...
    // Render search bar
    let search_line = if app.artisan_tab.search_mode {
        Line::from(vec![
            Span::styled("/", Style::default().fg(Theme::ACCENT)),
            Span::styled(
                app.artisan_tab.search_query.clone(),
                Style::default().fg(Theme::TEXT),
            ),
            Span::styled("█", Style::default().fg(Theme::ACCENT)),
        ])
    } else if !app.artisan_tab.search_query.is_empty() {
        Line::from(vec![
    ...
    let items: Vec<ListItem> = commands
        ...
        .map(|(idx, (name, _full_cmd, is_favorite))| {
            let is_selected = idx == app.artisan_tab.selected_command;
    ...
    state.select(Some(app.artisan_tab.selected_command));

    frame.render_stateful_widget(list, list_area, &mut state);
}

fn render_command_details(frame: &mut Frame, area: Rect, app: &App) {
    let scroll_offset = app.artisan_tab.details_scroll_offset;
    let favorites = app
        .config
        .as_ref()
        .map(|c| c.artisan_favorites())
    ...
    if let Some(cmd) = app.artisan_tab.selected_artisan_command(favorites) {
...
fn render_input_area(frame: &mut Frame, area: Rect, app: &App) {
    let is_input_mode = app.artisan_tab.input_mode;
    let is_running = app.artisan_tab.running_command.is_some();
    let is_empty = app.artisan_tab.input_buffer.is_empty();
    let favorites = app
        .config
        .as_ref()
        .map(|c| c.artisan_favorites())
    ...
    // Check if selected artisan command has required arguments
    let has_required_args = app
        .artisan_tab
        .selected_artisan_command(favorites)
    ...
    } else if let Some(cmd) = app.artisan_tab.selected_artisan_command(favorites) {
        if !cmd.arguments.is_empty() {
    ...
    let content = if is_input_mode {
        Line::from(vec![
            Span::styled(
                app.artisan_tab.input_buffer.clone(),
                Style::default().fg(Theme::TEXT),
            ),
            Span::styled("█", Style::default().fg(Theme::ACCENT)),
        ])
...
fn render_output_area(frame: &mut Frame, area: Rect, app: &App) {
    let title = if let Some(ref cmd) = app.artisan_tab.running_command {
    ...
    let scroll_offset = app.artisan_tab.output_scroll_offset;
